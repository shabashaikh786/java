public class Travels{
	public static void main(String args[]){
		
		int  num[] = {1,2,34,57,66};
		String name[] = {"suresh", "ramesh", "reva", "suma"};
		long  totalTraveler[] = {20000,87766,986765,98765443,9877665};
		char initial[]= {'S','R', 'R','S'};
		
		for(int index =0; index<num.length; index++){
			
			System.out.println("array elements number is:"+num[index]);
		}
		
		for(int index =0; index<name.length; index++){
		
			System.out.println("array elements name is:"+name[index]);
		}
		
		for(int index =0; index<totalTraveler.length; index++){
		System.out.println("array elements total Traveler is:"+totalTraveler[index]);
		}
		
		for(int index =0; index<initial.length; index++){
			System.out.println("array elements number is:"+initial[index]);
			
		}
		
		
	}

}